...
export default React.memo(function DashboardHeader() {
  ...
  const handlePrefetch = useCallback(
    (href: string) => {
      switch (href) {
        case "/overview":
          void utils.account.list.prefetch();
          void utils.transaction.list.prefetch({ limit: 100 });
          break;
        case "/accounts":
          void utils.account.list.prefetch();
          break;
        case "/transactions":
          void utils.transaction.list.prefetch({ limit: 20, page: 1 });
          break;
        case "/analytics":
          void utils.transaction.list.prefetch({ limit: 100 });
          break;
        case "/reports":
          void utils.report.list.prefetch({ limit: 100 });
          break;
        case "/settings":
          void utils.account.list.prefetch();
          void utils.category.list.prefetch();
          break;
        case "/profile":
            ...
            {secondaryLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  className={cn(
                    "text-primary-foreground/70 hover:text-primary-foreground relative px-3 py-2 text-sm font-medium whitespace-nowrap transition-colors",
                    isActive &&
                      "text-primary-foreground after:absolute after:right-3 after:bottom-0 after:left-3 after:h-0.5 after:rounded-full after:bg-white",
                  )}
                >
                  {link.label}
                </Link>
